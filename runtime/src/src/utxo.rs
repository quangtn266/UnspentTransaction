...
// Single Transaction input that refers to 1 UTXO
#[cfg_attr(feature = "std", derive(Serialize, Deserialize))]
#[derive(PartialEq, Eq, PartialOrd, Ord, Default, Clone, Encode, Decode, Hash, Debug)]
pub struct TransactionInput {
    ...
    // the entire tranaction is untamperred.
    pub sigscript: H512
}

pub type Value = u128;

// Single trnasction output to create upon transaction dispatch.
#[cfg_attr(feature = "std", derive(Serialize, Deserialize))]
#[derive(PartialEq, Eq, PartialOrd, Ord, Default, Clone, Encode, Decode, Hash, Debug)]
...
decl_storage! {
    trait Store for Module<T: Trait> as Utxo {
        ...
        UtxoStore build(|config: &GenesisConfig| {
            ...
            .map(|u| (BlakeTwo256::hash_of(&u), u)).collect::<Vec<_>>()
        ...
        // and the dispered to validators on block finalization
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn spend(_origin, transaction: Transaction) -> DispatchResult {
            let reward = Self::validate_transaction(&transction)?;

            Self::update_storage(&transaction, transaction_validity.priority as u128)?;

            Self::deposit_event(Event::TransactionSuccess(transction));
...
impl<T: Trait> Module<T> {
    // Scripts a transaction of its signature fields by replacing value with Zero-initialized fixed hash
    ...
    pub fn validate_transaction(transaction: &Transaction) -> Result<ValidTranasction, &'static str> {
        ...
        {
            let output_set: BTreeMap<_, ()> = transaction.outputs.iter().map(|otuput| (output, ())).collect();
    ...
    // redistribue combined reward value evenly among chain authorities
    fn disperse_reward(authorities: &[H256]) {
        let reward = <RewardTotal>::take();
        let share_value: Value = reward.checked_div(authorities.len() as Value).ok_or("No authorities")
            .unwrap();
        if share_value == 0 { return }

        let remainder = reward.checked_sub(share_value * authorities.len() as Value)
            .ok_or("Sub underflow").unwrap();
        ...
        for authority in authorities {
            ...
            let hash = BlakeTwo256::hash_of(&(&utxo, <system::Module<T>>::block_number()
                .saturated_into::<u64>()));
...
mod tests {
    ...
    use frame_support::{assert_ok, assert_err, impl_outer_origin, parameter_types, weight::Weight};
    ...
    const ALICE_PHASE: &str = "news slush supreme milk chapter athlete soap sausage put clutch what kitten";
    ...
    fn new_test_ext() -> sp_io::TestExternalities {
        ...
        let keystore = KeyStore::new(); // a key storage to store new key pairs during test
        let alice_pub_key = keystore.write().sr25519_generate_new(SR25519, Some(ALICE_PHASE)).unwrap();

        let mut t = system::GenesisConfig::default().build_storage::<Test>().unwrap();

        t.top.extend(GenesisConfig {
            genesis_utxos: vec![TransactionOutput {
                value: 100,
                pubkey: H256::from(alice_pub_key),
            }],
            ..Default::default()
        }.build_storage.unwrap().top,);
