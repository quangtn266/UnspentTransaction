...
fn remote_keystore(_url: &String) -> Result<Arcs<LocalKeyStore>, &'static str> {
...
pub fn new_full(mut config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    let (network, network_status_sink, system_rpc_tx, network_starter) =
    ...
    let backoff_authoring_blocks: Options<()> = None;
    let name = config.network.node_name.clone();
    let enable_grandpa = !config.disable_grandpa;
    let prometheus_registry = config.prometheus_registry().cloned(); /// checking again*************
    ...
    if role.is_authority() {
        ...
        // ex: if it fails we take down the service with it
    ...
    let grandpa_config = sc_finality_grandpa::Config {
        // make this available through chainsec
...
// Builds new service for a light client
pub fn new_light(mut config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    if conifg.offchain_worker.enabled {
        sc_service::build_offchain_workers(
            &config, backend.clone(), task_manager.spawn_handle(), client.clone(), network.clone(),
        );
    }
